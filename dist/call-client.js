...
export class CallClient extends EventEmitter {
    ...
    tokenScheduler = null;
    destroyed = false;
    /** Bumped to cancel an in-flight `start()` after the user hangs up early. */
    startEpoch = 0;
    constructor(options) {
        super();
    ...
    async start(roomId, options) {
        ...
        try {
            ...
            if (epoch !== this.startEpoch || this.state !== "outgoing") {
            ...
            // Group calls / rejoins start ACTIVE — connect media now.
            // 1:1 RINGING waits for call-accepted before joining LiveKit.
    ...
    async hangUp(options) {
        ...
        if (!ctx) {
            // Cancel optimistic dialing (start API still in flight).
    ...
    async joinMediaAfterAccept() {
        const ctx = this.context;
        if (!ctx || this.state !== "outgoing") {
            return;
        }
        this.transition("connecting");
        try {
            await this.connectMedia(ctx.livekit, ctx.call.type);
        }
        catch {
            // connectMedia already finalizes + emits error
        }
    }
