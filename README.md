...
| --- | --- |
| `callState` / `activeCall` / `incomingInvite` | Current call |
| `start(roomId, { type })` | Outgoing call |
| `accept(callId, roomId?)` / `decline(callId?)` | Incoming call |
| `hangUp()` | End (creator) or leave (group) |
| `rejoin(roomId)` | Reconnect after reload |
| `ingestInvite(payload)` | Replay an invite buffered before `CallClient` existed |
| `ringTimeoutSeconds` | 1:1 ring duration from tenant settings |
| `setMicrophoneEnabled` / `setCameraEnabled` | Device toggles |
| `on("incoming" \| "stateChange" \| "localTrack" \| …)` | Events |

LiveKit refreshes connected participant tokens automatically. Gateway `POST …/token` is still used for **fresh joins** (accept / rejoin).

Pass `{ debug: true }` to `CallClient` for invite/media console logs.
